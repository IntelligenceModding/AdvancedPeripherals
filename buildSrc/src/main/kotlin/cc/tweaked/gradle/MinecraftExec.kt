...
abstract class ClientJavaExec : JavaExec() {
    ...
    private fun setTestProperties() {
        if (!clientDebug) systemProperty("advancedperipheralstest.client", "")
        if (renderdoc) environment("LD_PRELOAD", "/usr/lib/librenderdoc.so")
        systemProperty("advancedperipheralstest.gametest-report", testResults.get().asFile.absoluteFile)
