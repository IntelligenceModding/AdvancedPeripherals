...
class UltimateExpander
{
    ...
    private UltimateMonitorEntity origin;
    private int width;
    private int height;

    UltimateExpander( UltimateMonitorEntity origin )
    {
        this.origin = origin;
    ...
    private boolean expandIn( boolean useXAxis, boolean isPositive )
    {
        ...
        if( !(other instanceof UltimateMonitorEntity otherMonitor) || !origin.isCompatible( otherMonitor ) ) return false;
        ...
        if( !isPositive )
        {
            ...
            if( !(otherOrigin instanceof UltimateMonitorEntity originMonitor) || !origin.isCompatible( originMonitor ) )
            {
                return false;
            }
