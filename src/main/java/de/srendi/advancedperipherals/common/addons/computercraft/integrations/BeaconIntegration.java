...
public class BeaconIntegration implements APGenericPeripheral {
    ...
    public final String getPrimaryEffect(BeaconBlockEntity blockEntity) {
        return blockEntity.primaryPower == null ? null : ForgeRegistries.MOB_EFFECTS.getKey(blockEntity.primaryPower).toString();
    }

    @LuaFunction(mainThread = true)
    public final String getSecondaryEffect(BeaconBlockEntity blockEntity) {
        return blockEntity.secondaryPower == null ? null : ForgeRegistries.MOB_EFFECTS.getKey(blockEntity.secondaryPower).toString();
    }
}
