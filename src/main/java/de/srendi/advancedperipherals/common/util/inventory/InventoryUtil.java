...
public class InventoryUtil {
    ...
    public static int moveItem(IItemHandler inventoryFrom, IItemHandler inventoryTo, ItemFilter filter) {
        ...
        if (inventoryFrom instanceof IStorageSystemItemHandler storageSystemHandler) {
            for (int i = toSlot == -1 ? 0 : toSlot; i < (toSlot == -1 ? inventoryTo.getSlots() : toSlot + 1); i++) {
                ...
                transferableAmount += storageSystemHandler
                        .extractItem(filter, extracted.getCount() - inserted.getCount(), false).getCount();
        ...
        if (inventoryTo instanceof IStorageSystemItemHandler storageSystemHandler) {
            for (int i = fromSlot == -1 ? 0 : fromSlot; i < (fromSlot == -1
                    ? inventoryFrom.getSlots()
                    : fromSlot + 1); i++) {
                if (filter.test(inventoryFrom.getStackInSlot(i))) {
                    ...
                    transferableAmount += inventoryFrom
                            .extractItem(i, extracted.getCount() - inserted.getCount(), false).getCount();
                    if (transferableAmount >= filter.getCount())
                        break;
                }
            }
        ...
        for (int i = fromSlot == -1 ? 0 : fromSlot; i < (fromSlot == -1
            ...
            if (filter.test(inventoryFrom.getStackInSlot(i))) {
                ...
                amount -= inserted.getCount();
                transferableAmount += inventoryFrom.extractItem(i, extracted.getCount() - inserted.getCount(), false)
                        .getCount();
                if (transferableAmount >= filter.getCount())
                    break;
            }
    ...
    public static int moveFluid(IFluidHandler inventoryFrom, IFluidHandler inventoryTo, FluidFilter filter) {
        ...
        if (inventoryFrom instanceof IStorageSystemFluidHandler storageSystemHandler) {
            ...
            transferableAmount += storageSystemHandler
                    .drain(filter.setCount(inserted), IFluidHandler.FluidAction.EXECUTE).getAmount();
    ...
    @Nullable public static IItemHandler getHandlerFromName(@NotNull IComputerAccess access, String name) throws LuaException {
    ...
    @Nullable public static IItemHandler getHandlerFromDirection(@NotNull String direction, @NotNull IPeripheralOwner owner)
            throws LuaException {
