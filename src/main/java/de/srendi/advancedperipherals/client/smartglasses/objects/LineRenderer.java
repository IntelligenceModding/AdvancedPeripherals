...
public class LineRenderer extends Simple2DObjectRenderer<LineObject> {
    @Override
    public void render(LineObject line, GuiGraphics gui, DeltaTracker partialTick, boolean is3D) {
        ...
        if (!line.pixelated) {
            if (dy == 0) {
                bufferBuilder.addVertex(matrix, 0, -halfWidth, 0).setColor(r, g, b, a);
                bufferBuilder.addVertex(matrix, 0, halfWidth, 0).setColor(r, g, b, a);
                bufferBuilder.addVertex(matrix, dx, halfWidth, 0).setColor(r, g, b, a);
                bufferBuilder.addVertex(matrix, dx, -halfWidth, 0).setColor(r, g, b, a);
                return;
            }
            if (dx == 0) {
                bufferBuilder.addVertex(matrix, -halfWidth, 0, 0).setColor(r, g, b, a);
                bufferBuilder.addVertex(matrix, halfWidth, 0, 0).setColor(r, g, b, a);
                bufferBuilder.addVertex(matrix, halfWidth, dy, 0).setColor(r, g, b, a);
                bufferBuilder.addVertex(matrix, -halfWidth, dy, 0).setColor(r, g, b, a);
                return;
            }
            float l = (float) Math.sqrt(dx * dx + dy * dy);
            float rx = -dy / l * halfWidth, ry = dx / l * halfWidth;
            bufferBuilder.addVertex(matrix, -rx, -ry, 0).setColor(r, g, b, a);
            bufferBuilder.addVertex(matrix, rx, +ry, 0).setColor(r, g, b, a);
            bufferBuilder.addVertex(matrix, dx + rx, dy + ry, 0).setColor(r, g, b, a);
            bufferBuilder.addVertex(matrix, dx - rx, dy - ry, 0).setColor(r, g, b, a);
            return;
        }
        ...
        for (int i = 0; i <= numPixels; i++) {
            ...
            bufferBuilder.addVertex(matrix, x1, y2, 0).setColor(r, g, b, a); // Bottom-left
            bufferBuilder.addVertex(matrix, x2, y2, 0).setColor(r, g, b, a); // Bottom-right
            bufferBuilder.addVertex(matrix, x2, y1, 0).setColor(r, g, b, a); // Top-right
            bufferBuilder.addVertex(matrix, x1, y1, 0).setColor(r, g, b, a); // Top-left
        }
    }
}
