...
object ClientTestHooks {
     ...
     */
    private val enabled: Boolean = System.getProperty("advancedperipheralstest.client") != null
    ...
    private fun openWorld() {
        ...
        minecraft.options.setSoundCategoryVolume(SoundSource.MUSIC, 0f)
        minecraft.options.setSoundCategoryVolume(SoundSource.AMBIENT, 0f)
        ...
        if (minecraft.levelSource.levelExists(LEVEL_NAME)) {
            ...
            val registryAccess = RegistryAccess.builtinCopy().freeze()
            minecraft.createWorldOpenFlows().createFreshLevel(
                LEVEL_NAME,
                LevelSettings("Test Level", GameType.CREATIVE, false, Difficulty.EASY, true, rules, DataPackConfig.DEFAULT),
                registryAccess,
                registryAccess.registryOrThrow(Registry.WORLD_PRESET_REGISTRY).getHolderOrThrow(WorldPresets.FLAT).value().createWorldGenSettings(RandomSource.create().nextLong(), false, false)
            )
        }
    }
